...
export function renderHist(){var c=$("trackList"),h=getHist();if(!h.length){showEmpty();return}var html='<div style="padding:8px 0"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><span style="font-size:14px;font-weight:600">搜索历史</span><button onclick="App.clearHist()" style="border:none;background:none;color:var(--text3);font-size:12px;cursor:pointer">清空</button></div><div style="display:flex;flex-wrap:wrap;gap:8px">';h.forEach(function(k){html+='<span class="hist-chip" data-kw="'+esc(k)+'" style="padding:6px 14px;border-radius:16px;background:var(--hover);color:var(--text2);font-size:12px;cursor:pointer;transition:all .2s">'+esc(k)+'</span>'});html+='</div></div>';c.innerHTML=html}

export var sTimer;
export function search(kw,page,append){if(!kw.trim()){showEmpty();return}page=page||1;S.kw=kw;showLoading();var u="/api/search?keyword="+encodeURIComponent(kw)+"&page="+page;if(S.src!=="all")u+="&source="+S.src;api(u,function(e,songs){if(e){toast("搜索失败");showEmpty();return}songs=songs||[];S.page=page;if(append){S.results=S.results.concat(songs)}else{S.results=songs}$("pageTitle").textContent="搜索结果";$("pageSub").textContent='搜索 "'+kw+'" 共 '+S.results.length+" 首";renderList(S.results,"search");renderLoadMore(songs.length===20)})}
...
export function initSearchEvents(){
  $("searchInput").addEventListener("input",function(e){clearTimeout(sTimer);sTimer=setTimeout(function(){if(S.tab==="search"){if(e.target.value.trim())search(e.target.value);else renderHist()}},300)});
